...
- [a-claude-session-resumed-from-a-spexr-dark-factory-card-runs](a-claude-session-resumed-from-a-spexr-dark-factory-card-runs.md) — A Claude session resumed from a SPEXR Dark Factory card runs with its cwd inside the worktree it was started in; removing that worktree pulls the directory out from under the session, so remove it last (or from a session outside it).

...
- **AC-3** The account resolved by `resolveAccount` feeds `CLAUDE_CONFIG_DIR` into `TerminalWidget.env`. The account is the launch profile named by `spexr.claude.activeProfile`, or the machine's only account when there is one — counting the default account alongside the configured launch profiles. When more than one account exists and none is chosen the user is prompted once and the answer is persisted folder-scoped against the **first workspace root** — the folder this terminal runs in. Separate workspaces therefore run under separate identities; the folders of one multi-root workspace share the side agent's account, since there is one agent terminal per window. Dismissing the prompt cancels the launch. `Spexr: Select Claude account` changes it later.

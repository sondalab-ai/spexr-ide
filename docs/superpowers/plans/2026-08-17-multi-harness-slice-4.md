...
- [ ] Move `ParsedTranscript` interface from `node/darkfactory/transcript-parser.ts`
      to `common/harness/harness-types.ts`.
- [ ] `node/darkfactory/transcript-parser.ts` re-exports it (one-line shim, as in
      Slice 1's resume-args shim). All existing importers keep working.
- [ ] Verify: typecheck + lint + full vitest green; no diff in test output.
...
- [ ] Add to `HarnessAdapter`: `listSessions(): Promise<HarnessSessionRef[]>`,
...
- [ ] `claudeHarness.listSessions`: extract today's `scanDisk` body from the
...
- [ ] `claudeHarness.parseTranscript`: call existing `parseTranscript(lines)` on
      the ref's bounded read; entries normalization is identity for Claude.
- [ ] `claudeHarness.followSession`: wrap the existing `fs.watch` tail logic
      (extracted from `startFollow`) into a `FollowHandle`.
- [ ] Backend service still behaves identically — it may keep using its own
...
- [ ] New `common/harness/opencode-harness.ts`.
- [ ] `isResumableId`: `/^ses_[A-Za-z0-9]+$/`.
- [ ] `buildResumeArgs(id, fork)`: `["--session", id]` + `"--fork"` when fork.
- [ ] `listSessions()`: `execFile("opencode", ["db", "--format", "json",
...
- [ ] `parseTranscript(ref)`: `execFile("opencode", ["export", ref.sessionId])`,
      ...
      `interactive: true`.
- [ ] `followSession`: throw `Error("not implemented")`.
- [ ] Tests (`opencode-harness.test.ts`) with captured fixtures: id whitelist,
...
- [ ] If Task 3's direct-field computation diverges from what
...
- [ ] Tests: a captured export maps to entries that produce correct
...
- [ ] Add a `DetectFn` seam to `DarkfactoryDeps` (default: resolve each harness
      binary via login-shell `command -v`, cached).
- [ ] Backend resolves `installedHarnesses([claudeHarness, opencodeHarness],
      detect)`; `listTranscripts` merges every installed harness's
      `listSessions()`; `liveDirs` uses the union of installed harnesses'
      `processNames()`.
- [ ] Tile pipeline consumes the merged refs: Claude refs keep their bounded
      read; opencode refs parse lazily (only within `RECENT_LIMIT`), via
      `harness.parseTranscript`.
- [ ] `planFocus` for opencode sessions: `configDir` is `""` (no config-dir
      override); resumable = always (opencode has no per-account dir mismatch).
- [ ] Tests: Claude-only merge identical to today; two-harness merge; opencode
...
- [ ] `darkfactory-terminal-manager.ts`: select harness by id shape
      (`/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/` →
      claude, `ses_…` → opencode); use that harness's `isResumableId` /
      `buildResumeArgs`.
- [ ] Opencode resume shell line: `cd <projectPath>; opencode --session <id>
      [--fork]; exec "$SHELL" -i` (login shell, no `CLAUDE_CONFIG_DIR` export).
- [ ] Terminal id stays `spexr-df-${sessionId}` (unique per session across
      harnesses — ids can't collide: UUID vs `ses_…`).
- [ ] Tests: opencode line shape; claude line unchanged; invalid id → undefined.

Commit: `feat(darkfactory): resume terminals select harness per session`

### Task 7 — Verification sweep + docs

- [ ] Full gate: `pnpm run typecheck && pnpm run lint && pnpm exec vitest run`.
- [ ] Manual check (if opencode installed): open the wall, confirm opencode
      tiles appear with goal/action line; click one → resume terminal works.
- [ ] Update design doc: mark Slice 4 status; note `followSession` stub is
      consumed in Slice 5.
- [ ] Mark spec 0013 checkboxes/status per repo convention (`in-progress` until
      merged).
